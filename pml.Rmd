...
First, create training and evaluation sets from the cleaned training data, in order to perform cross validation. 

```{r echo=TRUE} 
  inTrain <- createDataPartition(y=training_clean$classe, p=0.75, list=FALSE)
  training <- training_clean[inTrain,]
  evaluation <- training_clean[-inTrain,]
```
...
```{r echo=TRUE}
  pred1 <- predict(modFit1, evaluation) 
  confusionMatrix(pred1, evaluation$classe)
```
